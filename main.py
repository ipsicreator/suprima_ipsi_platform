...
with col1:
    st.markdown('<div class="card diag-up"><h2>나의 입시위치 진단</h2><p>PDF 기반 정밀 분석</p></div>', unsafe_allow_now=True)
    if st.button("📊 진단 서비스 시작"):
        st.switch_page("pages/01_입시위치_진단.py")
with col2:
    st.markdown('<div class="card diag-down"><h2>AI 탐구/세특 생성기</h2><p>차별화된 학생부 문구 생성</p></div>', unsafe_allow_now=True)
